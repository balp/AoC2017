...
c dec -10 if a >= 1
c inc -20 if c == 10"""
    val puzzle = """ebu inc 626 if iq < 0
...
wui inc -120 if i > -2038"""
    ...
    val registers: HashMap<String, Int> = runInstructions(example)
    // find max in registers
    println(registers)
    println(registers.maxBy { it.value })

    println(runInstructions(puzzle).maxBy { it.value })
}
...
private fun runInstructions(example: String): HashMap<String, Int> {
    ...
    for (line in example.lines()) {
        ...
        if (instrucationParser.matches(line)) {
            ...
            print(instruction + ":")
            val value = matches?.groups?.get(3)?.value
            print(value + ":")
            val conditionRegister = matches?.groups?.get(4)?.value
            ...
            if (operate) {
                when (instruction) {
                    "inc" -> registers[register!!] = registers[register]!! + value!!.toInt()
                    "dec" -> registers[register!!] = registers[register]!! - value!!.toInt()
                }
