...
<{o"i!a,<{i<a>"""
    ...
    for (line in garbage_example.lines()) {
        println(line)
        val score = proccessStream(line)
        println(score)
    }
    for (line in group_example.lines()) {
        println(line)
        val score = proccessStream(line)
        println(score)
    }
    for (line in score_example.lines()) {
        println(line)
        val score = proccessStream(line)
        println(score)
    }
    for (line in garbage_remove_example.lines()) {
        println(line)
        val score = proccessStream(line)
        println(score)
    }
    println(proccessStream(puzzle))
...
interface State {
    fun transfer(value: Char) : State
}

class InitialState(scoreCounter: ScoreCounter, garbageCounter: ScoreCounter) : State {
    val scoreCounter = scoreCounter
    val garbageCounter = garbageCounter
    override fun transfer(value: Char) : State  {
        print(" initial " + value)
        when (value) {
            '!' -> return CancelState(this)
            '{' -> return GroupState(this, 1, scoreCounter, garbageCounter)
            '<' -> return GarbageState(this, garbageCounter)
        }
        return this
    }
}

class GarbageState(parent: State, garbageCounter: ScoreCounter) : State {
    val parent = parent
    val garbageCounter = garbageCounter
    override fun transfer(value: Char) : State  {
        print(" garbage " + value)
        ...
        garbageCounter.add(1)
        return this
    }
}

class GroupState(parent: State,
                 depth : Int,
                 scoreCounter: ScoreCounter,
                 garbageCounter: ScoreCounter) : State {
    val parent = parent
    val depth = depth
    val scoreCounter = scoreCounter
    val garbageCounter = garbageCounter
    init {
        scoreCounter.add(depth)
    }
    override fun transfer(value: Char) : State  {
        print(" group(" + depth + ") "+ value)
        when (value) {
            '!' -> return CancelState(this)
            '<' -> return GarbageState(this, garbageCounter)
            '{' -> return GroupState(this, depth + 1, scoreCounter, garbageCounter)
            '}' -> return parent
        }
        return this
    }
...
class CancelState(parent: State) : State {
    val parent = parent
...
private fun proccessStream(stream: String): Int {
    val score = 0
    val scoreCounter = ScoreCounter()
    val garbageCounter = ScoreCounter()
    var state: State = InitialState(scoreCounter, garbageCounter)
    for (letter in stream) {
        state = state.transfer(letter)
    }
    println()
    println(garbageCounter.score)
