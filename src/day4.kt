...
oiii ioii iioi iiio"""
    val puzzle = """kvvfl kvvfl olud wjqsqa olud frc
...
rvbu czwpdit vmlihg spz lfaxxev zslfuto oog dvoksub"""
    ...
    println("into: " + validatePasswords2(test))
    println("puzzle: " + validatePasswords2(puzzle))

}

private fun partOne() {
    val test = """aa bb cc dd ee
aa bb cc dd aa
aa bb cc dd aaa"""
    val puzzle = """kvvfl kvvfl olud wjqsqa olud frc
slhm rdfm yxb rsobyt rdfm
...
rvbu czwpdit vmlihg spz lfaxxev zslfuto oog dvoksub"""
    ...
    println("into: " + validatePasswords(test))
    println("puzzle: " + validatePasswords(puzzle))
}

private fun validatePasswords(test: String): Int {
    var ok = 0
...
private fun validatePasswords2(test: String): Int {
    var ok = 0
    for (line in test.lines()) {
        if (verifyPassword2(line)) {
            println("ok: " + line)
            ok++
        } else {
            println("bad: " + line)
...
fun verifyPassword(passwd: String) : Boolean {
    val words = passwd.split("\\s+".toRegex())
    val counts = countWords(words)
    if (counts.any { it.value > 1 }) {
        return false
    }
    return true
}

fun verifyPassword2(passwd: String) : Boolean {
    val words = passwd.split("\\s+".toRegex())
    val counts = countWords2(words)
    println(counts)
    if (counts.any { it.value > 1 }) {
        return false
    }
...
private fun countWords(words: List<String>): Map<String, Int> {
    return words.groupingBy { it }
            .eachCount()
}

private fun countWords2(words: List<String>): Map<String, Int> {
    return words.groupingBy { sortChars(it) }
            .eachCount()
}
